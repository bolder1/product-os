    ...
    <div className="rounded-xl border border-white/[0.08] bg-white/[0.02] overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-medium text-[#94A3B8]">Analytics Overlay</span>
          {contextInsights.length > 0 && (
            <span className="text-[0.5625rem] px-1.5 py-0.5 rounded-full bg-amber-500/10 text-amber-400">
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            {/* Mini metrics */}
            <div className="grid grid-cols-3 gap-2 px-4 py-3 border-t border-white/[0.04]">
              {metrics.map((m) => (
                <div key={m.label} className="rounded-lg bg-white/[0.03] border border-white/[0.06] p-2 text-center">
                  <div className="flex items-center justify-center gap-1 mb-1">
                    <span className="text-[#64748B]">{m.icon}</span>
                  </div>
                  <p className="text-sm font-bold text-[#F1F5F9]">{m.value}</p>
                  <p className="text-[0.5625rem] text-[#64748B]">{m.label}</p>
                  <div className={`flex items-center justify-center gap-0.5 text-[0.5625rem] mt-0.5 ${
                    m.change > 0 ? 'text-emerald-400' : m.change < 0 ? 'text-rose-400' : 'text-[#64748B]'
                  }`}>
                    {m.change > 0 ? (
            ...
            {contextInsights.length > 0 && (
              <div className="border-t border-white/[0.04] px-4 py-2.5">
                <p className="text-[0.625rem] uppercase tracking-wider text-[#475569] mb-2">
                  Related Insights
                </p>
                <div className="space-y-1.5">
                  {contextInsights.map((insight) => (
                    <div
                      key={insight.id}
                      className="flex items-start gap-2 text-[0.6875rem]"
                    >
                      ...
                      <span className="text-[#94A3B8] leading-snug">{insight.text}</span>
                    </div>
                  ))}
                </div>
              </div>
