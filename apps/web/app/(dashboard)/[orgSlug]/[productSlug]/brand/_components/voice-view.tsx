      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={() => { setDraft(phrase.text); setEditing(true) }}
            className="text-[10px] text-[#64748B] hover:text-[#F1F5F9] px-1.5 py-0.5 rounded transition-colors">Edit</button>
          <button onClick={onRemove} className="text-[#64748B] hover:text-[#EF4444] transition-colors"><X size={12} /></button>
        </div>
      </div>
      {editing ? (
        <div className="flex flex-col gap-2">
          ...
          <div className="flex gap-1.5 justify-end">
            ...
            <button onClick={() => { onUpdate({ ...phrase, text: draft.trim() }); setEditing(false) }}
              className="text-xs px-2 py-1 rounded-md bg-[#8B5CF6] text-white hover:bg-[#7C3AED] transition-colors">Save</button>
          </div>
        </div>
      ) : (
    ...
    <div className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg bg-[#3B82F6]/15 flex items-center justify-center"><Sparkles size={14} className="text-[#3B82F6]" /></div>
          <div>
        ...
        {SAMPLE_CONTEXTS.map((ctx) => (
          <button key={ctx} onClick={() => { setContext(ctx); setPreview(null) }}
            className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-colors capitalize ${
              context === ctx ? 'bg-[#3B82F6] text-white' : 'bg-white/[0.04] text-[#64748B] hover:bg-white/[0.07] hover:text-[#94A3B8]'
            }`}>{ctx}</button>
        ))}
      </div>
      <button onClick={generate} disabled={generating}
        className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium transition-colors mb-3 ${
          generating ? 'bg-white/[0.04] text-[#64748B] cursor-not-allowed' : 'bg-[#3B82F6] text-white hover:bg-[#2563EB]'
        }`}>
      ...
      <div className="flex items-center justify-end gap-2 px-6 py-3 border-b border-white/[0.06]">
        ...
        <motion.button onClick={handleSave} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
          className={`flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-xs font-medium transition-colors ${
            saved ? 'bg-[#10B981] text-white' : 'bg-[#8B5CF6] text-white hover:bg-[#7C3AED]'
          }`}>
        ...
        <div className="max-w-5xl mx-auto px-6 py-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="flex items-center gap-2 px-2">
              ...
              <span className="px-2 py-0.5 rounded-full bg-white/[0.06] text-xs font-mono text-[#94A3B8]">v{voice.version}</span>
              <span className="text-xs text-[#64748B]">· Last updated {new Date(voice.updatedAt).toLocaleDateString()}</span>
            </div>
          </div>
        </div>
      </div>
