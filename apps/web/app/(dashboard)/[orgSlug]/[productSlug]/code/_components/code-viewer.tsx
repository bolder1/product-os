...
export function CodeViewer({ openFiles, activeFile, onSelectFile, onCloseFile }: CodeViewerProps) {
  ...
  if (!activeFile) {
    return (
      <div className="flex-1 flex items-center justify-center rounded-2xl bg-[#0a0f1e] border border-white/[0.08]">
        <div className="text-center">
          <div className="w-12 h-12 rounded-xl bg-[#06B6D4]/10 flex items-center justify-center mx-auto mb-3">
            <span className="text-[#06B6D4] text-xl font-mono">{'{}'}</span>
          </div>
          <p className="text-[#94A3B8] text-sm">Select a file to view its code</p>
          <p className="text-[#64748B] text-xs mt-1">Click any file in the tree</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex-1 flex flex-col rounded-2xl bg-[#0a0f1e] border border-white/[0.08] overflow-hidden">
      {/* Tab bar */}
      {openFiles.length > 0 && (
            ...
            return (
              <div
                ...
                className={`flex items-center gap-2 px-3 py-2 text-xs border-r border-white/[0.06] cursor-pointer transition-colors min-w-0 ${
                  isActive
                    ? 'bg-[#0a0f1e] text-[#F1F5F9] border-b-2 border-b-[#06B6D4]'
              ...
              >
                ...
                <button
                  ...
                  className="flex-shrink-0 hover:text-[#F1F5F9] transition-colors"
                >
                  <X className="w-3 h-3" />
                </button>
              </div>
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-white/[0.06]">
        <div className="flex items-center gap-1 text-xs">
          {pathParts.map((part, i) => (
            <span key={i} className="flex items-center gap-1">
              {i > 0 && <ChevronRight className="w-3 h-3 text-[#64748B]" />}
              <span
                className={
        ...
        <div className="flex items-center gap-1.5">
          <button
            onClick={handleCopy}
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs text-[#94A3B8] hover:text-[#F1F5F9] bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.08] transition-colors"
          >
          ...
          <button className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs text-[#8B5CF6] bg-[#8B5CF6]/10 hover:bg-[#8B5CF6]/20 border border-[#8B5CF6]/20 transition-colors">
          ...
          >
            ...
            <div className="flex-shrink-0 py-4 pr-4 pl-4 text-right select-none border-r border-white/[0.04]">
              {highlighted.map((_, i) => (
                <div
                  key={i}
                  className="text-xs leading-6 text-[#64748B]/50 font-mono"
                >
                  {i + 1}
                </div>
              ))}
