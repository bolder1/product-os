  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[9px] text-[#64748B] ml-1">Describe your context — AI adapts the variables</span>
      </div>
          ...
          <button
            ...
            className={`text-[9px] px-2 py-0.5 rounded-full border transition-all ${
              prompt === preset
                ...
                : 'border-white/[0.08] text-[#64748B] hover:border-white/[0.18] hover:text-[#94A3B8]'
            }`}
          >
            {preset}
          </button>
      ...
      <div className="flex gap-2">
        <textarea
          ...
          rows={2}
          className="flex-1 px-3 py-2 rounded-lg bg-white/[0.03] border border-white/[0.08] text-[11px] text-[var(--text-primary)] placeholder:text-[#475569] outline-none focus:border-[#8B5CF6]/40 resize-none"
        />
        <button
          onClick={handleRemix}
          disabled={!prompt.trim() || remixMutation.isPending}
          className="px-3 py-2 rounded-lg bg-[#8B5CF6]/20 border border-[#8B5CF6]/30 text-[#C4B5FD] text-[10px] font-medium hover:bg-[#8B5CF6]/30 disabled:opacity-40 disabled:cursor-not-allowed transition-all flex items-center gap-1.5 shrink-0 self-start"
        >
      ...
      <AnimatePresence>
        {suggestions && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 6 }}
            className="rounded-xl border border-[#8B5CF6]/20 bg-[#8B5CF6]/5 p-3 space-y-2.5"
          >
            {changedVars.length === 0 ? (
              <p className="text-[10px] text-[#64748B] text-center py-1">
                AI found no variable changes for this prompt. Try being more specific.
              </p>
            ) : (
              <>
                <p className="text-[10px] font-medium text-[#C4B5FD]">
                ...
                <div className="space-y-1.5">
                  {changedVars.map((v) => (
                    <div key={v.key} className="flex items-center gap-2 text-[10px]">
                      <span className="text-[#64748B] w-28 shrink-0 truncate">{v.label}</span>
                      <span className="text-[#475569] line-through truncate max-w-[80px]">
                        {String(currentValues[v.key] ?? v.default)}
                      </span>
                      <span className="text-[#C4B5FD] font-medium truncate max-w-[100px]">
                ...
                <div className="flex items-center gap-2 pt-1">
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1 rounded-lg text-[10px] font-medium transition-all ${
                      applied
                        ? 'bg-[#10B981]/20 border border-[#10B981]/30 text-[#6EE7B7]'
                        : 'bg-[#8B5CF6]/20 border border-[#8B5CF6]/30 text-[#C4B5FD] hover:bg-[#8B5CF6]/30'
                    }`}
                  >
            ...
            {rawResponse && (
              <div>
                <button
                  ...
                  className="flex items-center gap-1 text-[9px] text-[#475569] hover:text-[#64748B] transition-colors"
                >
                  {showRaw ? <ChevronUp size={9} /> : <ChevronDown size={9} />}
                  {showRaw ? 'Hide' : 'Show'} raw AI response
                </button>
                {showRaw && (
                  <pre className="mt-1.5 text-[9px] text-[#64748B] bg-black/20 rounded-lg p-2 overflow-x-auto whitespace-pre-wrap">
                    {rawResponse}
                  </pre>
                )}
              </div>
