          ...
          <div className="flex items-center gap-3">
            {editingName ? (
              <input
                ...
                className="text-lg font-semibold text-[var(--text-primary)] bg-transparent border-b border-[var(--accent)]/50 focus:outline-none"
              />
            ) : (
              <h2
                onClick={() => {
                ...
                className="text-lg font-semibold text-[var(--text-primary)] cursor-pointer hover:text-[var(--accent)] transition-colors"
              >
                {component.name}
              </h2>
            )}
            ...
            <div className="relative">
              <select
                ...
                className="appearance-none pl-2 pr-6 py-1 rounded-md text-[10px] font-medium border border-white/[0.06] focus:outline-none focus:border-[var(--accent)]/40 cursor-pointer"
                style={{ backgroundColor: `${accent}15`, color: accent }}
              >
                {categories.filter((c) => c !== 'All').map((c) => (
                  <option key={c} value={c} className="bg-[#0c1125] text-[var(--text-primary)]">
                    {c}
                  </option>
                ))}
              </select>
              ...
              <div className="rounded-xl border border-white/[0.06] overflow-hidden">
                <div className="grid grid-cols-[1fr_80px_100px_60px_1fr] gap-px bg-white/[0.04]">
                  ...
                  {['Name', 'Type', 'Default', 'Req.', 'Description'].map((h) => (
                    <div key={h} className="px-3 py-2.5 bg-white/[0.02] text-[10px] font-medium text-[var(--text-tertiary)] uppercase tracking-wider">
                      {h}
                    </div>
                  ))}
                  {/* Rows */}
                  {component.props.map((prop, i) => (
                    <>
                      <div key={`${prop.id}-name`} className={`px-3 py-2.5 text-xs text-[var(--text-primary)] font-medium ${i % 2 ? 'bg-white/[0.01]' : 'bg-[var(--bg-base)]'}`}>
                        {prop.name}
                      </div>
                      <div key={`${prop.id}-type`} className={`px-3 py-2.5 ${i % 2 ? 'bg-white/[0.01]' : 'bg-[var(--bg-base)]'}`}>
                        <span className="text-[10px] px-1.5 py-0.5 rounded-md bg-[var(--accent)]/10 text-[var(--accent)] font-mono">
                          {prop.type}
                        </span>
                      </div>
                      <div key={`${prop.id}-default`} className={`px-3 py-2.5 text-xs text-[var(--text-secondary)] font-mono ${i % 2 ? 'bg-white/[0.01]' : 'bg-[var(--bg-base)]'}`}>
                        {prop.defaultValue || '—'}
                      </div>
                      <div key={`${prop.id}-req`} className={`px-3 py-2.5 text-center ${i % 2 ? 'bg-white/[0.01]' : 'bg-[var(--bg-base)]'}`}>
                        {prop.required ? (
                      ...
                      <div key={`${prop.id}-desc`} className={`px-3 py-2.5 text-xs text-[var(--text-tertiary)] ${i % 2 ? 'bg-white/[0.01]' : 'bg-[var(--bg-base)]'}`}>
                        {prop.description}
                      </div>
                    </>
                  ))}
            ...
            >
              ...
              {onSaveVersion && (
                <div className="flex items-center gap-2 p-3 rounded-lg border border-white/[0.06] bg-white/[0.02]">
                  <Save className="w-3.5 h-3.5 text-[var(--accent)] shrink-0" />
                  <input
                    value={versionLabel}
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 rounded text-[11px] font-medium bg-[var(--accent)]/15 text-[var(--accent)] hover:bg-[var(--accent)]/25 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                  >
              ...
              {versionHistory.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  <History className="w-5 h-5 text-[var(--text-tertiary)] mb-2" />
                  ...
                  {[...versionHistory].reverse().map((ver, i) => {
                    ...
                    return (
                      <div key={`${ver.label}-${i}`} className="relative flex gap-3 py-2.5 pl-1">
                        ...
                        >
                          <div className={`w-2 h-2 rounded-full ${isLatest ? 'bg-[var(--accent)]' : 'bg-white/[0.2]'}`} />
                        </div>

                        {/* Version info */}
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <span className={`text-xs font-medium ${isLatest ? 'text-[var(--accent)]' : 'text-[var(--text-primary)]'}`}>
                              {ver.label}
                            </span>
                            {isLatest && (
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            {ver.createdBy && (
                              <span className="text-[10px] text-[var(--text-tertiary)]">by {ver.createdBy.slice(0, 8)}</span>
                            )}
                          </div>
                        </div>
                        ...
                        {!isLatest && onRestoreVersion && (
                          <button
                            ...
                            className="shrink-0 flex items-center gap-1 px-2 py-1 rounded text-[10px] text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-white/[0.04] transition-colors"
                          >
