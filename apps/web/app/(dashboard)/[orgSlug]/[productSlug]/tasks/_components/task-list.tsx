...
export function TaskList({ tasks, onUpdateTask }: TaskListProps) {
  ...
  return (
    <div className="rounded-xl border border-white/[0.08] overflow-hidden flex flex-col">
      {/* Header row */}
      <div className="grid grid-cols-[40px_1fr_120px_100px_140px_100px_140px] gap-2 px-4 py-3 bg-white/[0.02] border-b border-white/[0.06] shrink-0">
      ...
      {sorted.length === 0 ? (
        ...
        <div
          ...
          style={{ height: 480 }}
          role="list"
          aria-label={`${sorted.length} tasks`}
          {...scrollProps}
        >
                ...
                >
                  ...
                  <div className="flex items-center justify-center">
                    <button
                      ...
                      aria-label={`${isSelected ? 'Deselect' : 'Select'} task: ${task.title}`}
