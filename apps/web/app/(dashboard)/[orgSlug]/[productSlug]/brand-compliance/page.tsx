  ...
  const violations: DriftViolation[] = [
    {
      ...
      objectId: 'comp-1',
      rule: 'Color must use brand token',
      detail: 'background-color: #2563eb — raw hex used instead of token --color-primary',
      suggestion: 'Replace #2563eb with var(--color-primary)',
      autoFixable: true,
      rawValue: '#2563eb',
      expectedToken: '--color-primary',
    },
    {
      id: `${productId}-v2`,
      severity: 'error',
      category: 'color',
      studio: 'design',
      objectName: 'Dashboard Screen',
      objectId: 'design-1',
      rule: 'Text color must use brand token',
      detail: 'color: #374151 — not mapped to a brand token',
      suggestion: 'Replace with var(--text-primary) or var(--text-secondary)',
      autoFixable: true,
      rawValue: '#374151',
    ...
    },
    {
      ...
      category: 'typography',
      studio: 'pages',
      objectName: 'Landing Page Hero',
      objectId: 'page-1',
      rule: 'Font family must match brand system',
      detail: "font-family: 'Arial' — not in approved brand typefaces",
      suggestion: "Replace with brand font: 'Inter', var(--font-sans)",
      autoFixable: false,
      rawValue: 'Arial',
      expectedToken: 'Inter',
    },
    {
      ...
      objectId: 'comp-2',
      rule: 'Border color should use token',
      detail: 'border-color: #d1d5db — should use --border-subtle',
      suggestion: 'Replace #d1d5db with var(--border-subtle)',
      autoFixable: true,
      rawValue: '#d1d5db',
      expectedToken: '--border-subtle',
    },
    {
      id: `${productId}-v5`,
      severity: 'warning',
      ...
      objectId: 'design-2',
      rule: 'Spacing must use 4pt grid',
      detail: 'padding: 14px — not on 4pt grid (should be 12px or 16px)',
      suggestion: 'Change to padding: 16px (spacing-4)',
      autoFixable: true,
      rawValue: '14px',
      expectedToken: 'spacing-4 (16px)',
    },
    {
      id: `${productId}-v6`,
      severity: 'warning',
      ...
      objectId: 'comp-3',
      rule: 'Font size must use type scale token',
      detail: 'font-size: 17px — not in brand type scale',
      suggestion: 'Use 16px (text-base) or 18px (text-lg) from type scale',
      autoFixable: true,
      rawValue: '17px',
      expectedToken: 'text-base (16px)',
    },
    {
      id: `${productId}-v7`,
      severity: 'warning',
      category: 'color',
      studio: 'graphics',
      objectName: 'Social Banner',
      objectId: 'graphic-1',
      rule: 'Gradient must use brand colors',
      detail: 'Linear gradient using #0ea5e9 → #8b5cf6, not brand palette colors',
      suggestion: 'Use --color-primary → --color-secondary gradient instead',
      autoFixable: false,
    },
    {
      id: `${productId}-v8`,
      ...
      objectName: 'Badge',
      objectId: 'comp-4',
      rule: 'Token is deprecated',
      detail: '--color-accent is deprecated; --accent-text is the current token',
      suggestion: 'Migrate to --accent-text across all usages',
      autoFixable: true,
      rawValue: '--color-accent',
      expectedToken: '--accent-text',
    },
    {
      id: `${productId}-v9`,
      severity: 'info',
      category: 'spacing',
      studio: 'pages',
      objectName: 'About Page',
      objectId: 'page-2',
      rule: 'Inconsistent section padding',
      detail: 'Section padding varies: 24px, 28px, 32px — not consistent',
      suggestion: 'Standardise to spacing-8 (32px) for section padding',
      autoFixable: false,
    },
    {
      id: `${productId}-v10`,
      ...
      category: 'component',
      studio: 'design',
      objectName: 'Profile Screen',
      objectId: 'design-3',
      rule: 'Uses detached component',
      detail: 'Avatar element is detached from Avatar component in Component Library',
      suggestion: 'Re-link to Avatar@v2 from Component Library',
      autoFixable: false,
    },
  ]

  // Simulate some already fixed based on seed (so score changes on "refresh")
  if (seed % 2 === 1) {
    return violations.filter((_, i) => i !== 0 && i !== 3)
  }
  return violations
}
...
function computeScore(violations: DriftViolation[]): number {
  const penalty = violations.reduce((p, v) => {
    if (v.severity === 'error') return p + 12
    if (v.severity === 'warning') return p + 5
    return p + 2
  }, 0)
...
function ViolationRow({
  ...
  return (
    <div
      className="flex items-start gap-3 p-4 border-b border-[var(--border-subtle)] last:border-0 hover:bg-[var(--bg-hover)] transition-colors"
    >
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-[var(--text-secondary)] leading-relaxed">
        ...
        {autoFixable > 0
          ? `${autoFixable} issue${autoFixable > 1 ? 's' : ''} can be auto-fixed with one click. `
          : ''}
...
export default function BrandCompliancePage() {
  ...
  const violationsByStudio = useMemo(() =>
    (Object.keys(STUDIO_CONFIG) as Studio[]).map((s) => ({
      studio: s,
      violations: allViolations.filter((v) => v.studio === s),
    })),
    [allViolations]
  )
  ...
  function handleFixAll() {
    ...
    setFixingId('bulk')
    setTimeout(() => {
      setFixedIds((prev) => new Set([...prev, ...fixable]))
      setFixingId(null)
    }, 2000)
  }
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden" style={{ background: 'var(--bg-base)' }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-base font-semibold text-[var(--text-primary)]">Brand Compliance</h1>
              <p className="text-xs text-[var(--text-tertiary)]">Detect token drift across all studios</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <select
              value={studioFilter}
              onChange={(e) => setStudioFilter(e.target.value as Studio | 'all')}
              ...
              {[...filtered]
                ...
                .map((v) => (
                  <ViolationRow
                    ...
                    violation={v}
                    onFix={handleFix}
                    fixing={fixingId === v.id || fixingId === 'bulk'}
                  />
                ))}
            </div>
          )}
      ...
      <div
        className="w-[280px] flex-shrink-0 border-l border-[var(--border-subtle)] flex flex-col overflow-y-auto"
        style={{ background: 'var(--bg-card)' }}
      >
        {/* Score */}
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-[10px] text-[var(--text-tertiary)] text-center">
              {allViolations.length === 0
                ...
                : `${allViolations.length} issue${allViolations.length !== 1 ? 's' : ''} affecting score`}
            </p>
          </div>
        </div>
