            ...
            {tab === "automations" && (
              <div className="flex flex-col gap-1">
                {automations.map((a) => (
                  ...
                  >
                    <Zap className={`w-3.5 h-3.5 shrink-0 mt-0.5 ${a.enabled ? "text-amber-400" : "text-[var(--text-tertiary)]"}`} />
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 p-2 rounded-lg border border-dashed border-white/[0.08] text-[11px] text-[var(--text-tertiary)] hover:text-emerald-400 hover:border-emerald-500/20 transition-colors justify-center"
                >
              ...
              <div className="flex flex-col gap-1">
                {forms.map((f) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-[11px] font-medium text-[var(--text-primary)] truncate">{f.name}</div>
                      <div className="text-[9px] text-[var(--text-tertiary)]">
            ...
            <motion.div key={`auto-${selectedAutomation.id}`} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
              ...
              <div className="flex items-center gap-3">
                ...
                <input value={selectedAutomation.name}
                  ...
                  className="text-lg font-semibold bg-transparent text-[var(--text-primary)] outline-none border-b border-transparent focus:border-amber-500/40 pb-1 flex-1" />
              ...
              <div className="p-3 rounded-lg bg-white/[0.02] border border-white/[0.06] space-y-2">
                ...
                <select value={selectedAutomation.triggerType}
                  ...
                  className="w-full bg-white/[0.05] text-sm text-[var(--text-secondary)] rounded-lg px-2 py-1.5 outline-none border border-white/[0.08]">
                  <option value="entity_created" className="bg-[var(--bg-inset)]">Entity Created</option>
                  <option value="entity_updated" className="bg-[var(--bg-inset)]">Entity Updated</option>
                  <option value="entity_deleted" className="bg-[var(--bg-inset)]">Entity Deleted</option>
                  <option value="state_changed" className="bg-[var(--bg-inset)]">State Changed</option>
                  <option value="schedule" className="bg-[var(--bg-inset)]">Schedule (Cron)</option>
                  <option value="webhook" className="bg-[var(--bg-inset)]">Webhook</option>
                  <option value="manual" className="bg-[var(--bg-inset)]">Manual</option>
                </select>
              </div>
              ...
              <div className="p-3 rounded-lg bg-white/[0.02] border border-white/[0.06] space-y-2">
                ...
                <p className="text-[10px] text-[var(--text-tertiary)]">Add field conditions to filter when this automation runs.</p>
              ...
              <div className="p-3 rounded-lg bg-white/[0.02] border border-white/[0.06] space-y-2">
                ...
                <p className="text-[10px] text-[var(--text-tertiary)]">Define what happens when this automation fires.</p>
            ...
            <motion.div key={`form-${selectedForm.id}`} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
              ...
              <div className="flex items-center gap-3">
                ...
                <input value={selectedForm.name}
                  ...
                  className="text-lg font-semibold bg-transparent text-[var(--text-primary)] outline-none border-b border-transparent focus:border-blue-500/40 pb-1 flex-1" />
              </div>

              {/* Entity binding */}
              <div className="p-3 rounded-lg bg-white/[0.02] border border-white/[0.06] space-y-2">
                <span className="text-[10px] font-semibold uppercase tracking-wider text-[var(--text-tertiary)]">Bound Entity</span>
                <select value={selectedForm.entityId}
                  onChange={(e) => setForms((prev) => prev.map((f) => f.id === selectedForm.id ? { ...f, entityId: e.target.value } : f))}
                  className="w-full bg-white/[0.05] text-sm text-[var(--text-secondary)] rounded-lg px-2 py-1.5 outline-none border border-white/[0.08]">
                  {entities.map((e) => <option key={e.id} value={e.id} className="bg-[var(--bg-inset)]">{e.name}</option>)}
                </select>
              </div>
              ...
              <div className="p-3 rounded-lg bg-white/[0.02] border border-white/[0.06] space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-[var(--text-tertiary)]">{selectedForm.fieldCount} fields</span>
                </div>
                {(() => {
                  const ent = entities.find((e) => e.id === selectedForm.entityId);
                  if (!ent) return <p className="text-[10px] text-[var(--text-tertiary)]">Select an entity to see available fields</p>;
                  return (
                    <div className="space-y-1">
                      {ent.fields.map((f) => (
                        <div key={f.id} className="flex items-center gap-2 p-1.5 rounded bg-white/[0.02] border border-white/[0.04]">
                          ...
                          <span className="text-[11px] text-[var(--text-primary)] flex-1">{f.name}</span>
                          <span className="text-[9px] text-[var(--text-tertiary)] bg-white/[0.04] px-1 py-0.5 rounded">{f.type}</span>
                          <select defaultValue="full" className="bg-white/[0.05] text-[9px] text-[var(--text-secondary)] rounded px-1 py-0.5 outline-none border border-white/[0.08]">
              ...
              <div className="p-3 rounded-lg bg-white/[0.02] border border-white/[0.06] space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-[var(--text-tertiary)]">{selectedForm.stepCount} step{selectedForm.stepCount !== 1 ? "s" : ""}</span>
                </div>
                <p className="text-[10px] text-[var(--text-tertiary)]">Split form fields across multiple wizard steps.</p>
              ...
              {/* Submit config */}
              <div className="p-3 rounded-lg bg-white/[0.02] border border-white/[0.06] space-y-2">
                <span className="text-[10px] font-semibold uppercase tracking-wider text-[var(--text-tertiary)]">On Submit</span>
                <select defaultValue="create"
                  className="w-full bg-white/[0.05] text-sm text-[var(--text-secondary)] rounded-lg px-2 py-1.5 outline-none border border-white/[0.08]">
                  <option value="create" className="bg-[var(--bg-inset)]">Create Entity</option>
                  <option value="update" className="bg-[var(--bg-inset)]">Update Entity</option>
                  <option value="transition" className="bg-[var(--bg-inset)]">Trigger Workflow Transition</option>
                </select>
              </div>
